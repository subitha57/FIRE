...
// Signin function (Sending OTP and generating token)
exports.Signin = async (req, res) => {
  //#swagger.tags = ['Login-User']
  const { email } = req.body;
...
// Token Validation
exports.Validate = [require("../Middleware/verifyToken"), async (req, res) => {
  //#swagger.tags = ['Login-User']
  ...
  try {
    ...
    res.json({ success: true, message: "Valid email and token", user });
  } catch (err) {
    console.error(err);
